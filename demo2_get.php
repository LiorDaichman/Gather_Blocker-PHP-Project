...
(min-device-width: 768px) and (max-device-width: 1024px)  {
	...
	*/
	...
	td:nth-of-type(5):before { content: "max-capacity:"; }
	td:nth-of-type(6):before { content: "category:"; }
	td:nth-of-type(7):before { content: "real-time:"; }
	td:nth-of-type(8):before { content: "external line:"; }
	td:nth-of-type(9):before { content: "occupation(%):"; }
	td:nth-of-type(10):before { content: "check in:"; }
...
$sql="SELECT `id`, `name`, `address`, `city`, `max-capacity`, `category`, `user-name`,`real-time`, TIME_FORMAT(`opening`, '%h:%i%p')`opening`,TIME_FORMAT(`closing`, '%h:%i%p') `closing` FROM `shops`  WHERE `category`='$q'";
...
while($row = mysqli_fetch_array($result)) {
  ...
  echo "<td>" . $int_cast , "% -Arrival is not recommended <i style='font-size: 24px; color: red;' class='fas fa-ban'></i>" . "</td>";
   ...
   echo "<td><button onclick='loadDocs(this.value)' class='checkbutton' name='subject' value=".$row['id'].">check in:".$row['id']."</button></td>";
  echo "</tr>";
  }
}
else {
